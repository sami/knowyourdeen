...
export function SharePopover() {
  ...
  return (
    <div className="relative" ref={popoverRef}>
      ...
      {open && (
        <div className="absolute top-full mt-2 right-0 bg-white rounded-2xl shadow-xl border border-gray-100 p-2 min-w-[180px] animate-fade-in-up z-50">
          <button
            onClick={handleWhatsApp}
            className="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-green-50 transition-colors text-left"
          >
            <WhatsAppIcon className="w-5 h-5 text-green-500" />
            <span className="font-semibold text-gray-700">{t('shareWhatsApp', lang)}</span>
          </button>
          <button
            onClick={handleCopy}
          ...
          >
            ...
            <span className="font-semibold text-gray-700">
              {copied ? t('copied', lang) : t('shareCopyLink', lang)}
            </span>
          </button>
        </div>
      )}
