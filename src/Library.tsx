...
export default function Library({ lang, onBack }: { lang: 'ar' | 'en', onBack: () => void }) {
  const isRTL = lang === 'ar';
  const [activeCategory, setActiveCategory] = useState<string>(CATEGORIES[0].id);
  const [expandedQuestionId, setExpandedQuestionId] = useState<number | null>(null);

  const currentQuestions = questionsDB.filter(
    (q) => {
      ...
      <div className="bg-white/80 backdrop-blur-md p-6 rounded-3xl shadow-xl flex-1 flex flex-col mb-4 overflow-hidden">
        ...
        <div className="flex items-center justify-between mb-6 pb-4 border-b border-teal-100/50">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-3xl font-extrabold text-teal-800">
              {lang === 'ar' ? 'مكتبة الأسئلة' : 'Questions Library'}
            </h2>
          </div>
          <button 
            onClick={onBack}
        ...
        <div className="flex overflow-x-auto gap-2 mb-6 pb-2 hide-scrollbar">
          {CATEGORIES.map(cat => (
            <button
              key={cat.id}
              onClick={() => { setActiveCategory(cat.id); setExpandedQuestionId(null); }}
        ...
        <div className="flex-1 overflow-y-auto pr-2 space-y-3 custom-scrollbar" dir={isRTL ? 'rtl' : 'ltr'}>
          {currentQuestions.map((q) => {
            ...
            return (
              <div 
                key={q.id} 
                className={`border-2 rounded-2xl overflow-hidden transition-all duration-300 ${isExpanded ? 'border-teal-400 bg-white shadow-lg' : 'border-teal-100/50 bg-white/50 hover:border-teal-200'}`}
              >
                <button 
                ...
                >
                  <div className="flex gap-3 text-left">
                    <span className="flex-shrink-0 w-8 h-8 rounded-full bg-teal-100 text-teal-800 flex items-center justify-center font-bold text-sm">
                      {q.id}
                    </span>
                    <h3 className={`font-bold text-gray-800 mt-1 ${isRTL ? 'text-right' : 'text-left'}`}>
                      {q[lang].q}
                    </h3>
                  </div>
                  <div className="mt-1 ml-2 mr-2 text-teal-500 flex-shrink-0">
                  ...
                  <div className="p-4 bg-teal-50/50 border-t border-teal-100">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
                      {Object.entries(q[lang].options).map(([letter, text]) => {
                        ...
                        return (
                          <div key={letter} className={`p-3 rounded-xl border ${isCorrect ? 'bg-green-100 border-green-300 text-green-900 font-bold' : 'bg-white border-gray-200 text-gray-600'}`}>
                            <span className="mr-2 ml-2">{letter} -</span>
                            {text}
                            {isCorrect && <span className={`float-right text-green-600 ${isRTL ? 'float-left' : 'float-right'}`}>✓</span>}
                          </div>
                        );
                      })}
                    </div>
                    <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                      <p className={`font-bold text-teal-800 mb-1 ${isRTL ? 'text-right' : 'text-left'}`}>{lang === 'ar' ? 'التفسير:' : 'Explanation:'}</p>
